...
def compute_orct1inverseV2(bayer, Alocal=None, precisionFloatingPoint=0):
    ...
    for row_index in range(0, bayer_number_of_rows, 2):
        for column_index in range(0, bayer_number_of_columns - 1, 2):
            ...
            final_block[row_index][column_index] = np.ceil(converted_gr_r[0])

            final_block[row_index + 1][column_index] = np.ceil(converted_gb_b[1])
